...
TH2D *hist2DMaker::histMake (Int_t strip_min, Int_t strip_max, Double_t pitch_size, const std::vector<hit_with_charge> *m_strip_position) {
    ...
    for(int j = 0; j<m_strip_position->size(); j++){
            hist->Fill(m_strip_position->at(j).x+pitch_size*0.5,0.5,m_strip_position->at(j).charge);
    }
    return hist;
